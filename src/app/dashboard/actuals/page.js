...
import { StatCard, SectionHeader, fmt, Badge } from '@/components/ui';
...
export default function Actuals() {
  ...
  const data = getPredictedVsActual(rep);
        ...
        <table className="w-full text-sm">
          ...
          <tbody>{data.map((d,i) => (
            <tr key={d.id+d.type} className={i%2===0?'bg-white':'bg-gray-50'}>
              ...
              <td className="px-3 py-2"><Badge text={d.type} color={d.type==='Assessment'?'blue':'gold'} /></td>
