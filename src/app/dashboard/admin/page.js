...
import { REPS, getRepStats, getRepPipelineForecast, getOpenDeals, withMonthsOpen, getRepFunnelStats, calcWeighted30, getMomentum, getWinStreak, getRevenueVelocity, getForecastAccuracy, getPipelineCoverage, getRevByPipeline } from '@/data/utils';
import { StatCard, SectionHeader, fmt, pct, Select, FilterBar, Badge, TableWrapper, EmptyState } from '@/components/ui';
...
export default function AdminDeepDive() {
  ...
  return (
    <div>
      ...
      {aOpen.length===0?<EmptyState message="No open assessment deals." icon="📋" />:(
        <TableWrapper><table className="w-full text-sm"><thead><tr className="bg-gray-100">
          <th className="px-3 py-2.5 text-left font-semibold">Deal</th><th className="px-3 py-2.5 text-right font-semibold">Value</th><th className="px-3 py-2.5 text-center font-semibold">Pipe</th>
          <th className="px-3 py-2.5 text-center font-semibold">Close%</th><th className="px-3 py-2.5 text-right font-semibold">30d$</th><th className="px-3 py-2.5 text-center font-semibold">Open</th><th className="px-3 py-2.5 text-center font-semibold">Status</th>
        </tr></thead><tbody>{aOpen.map((d,i)=>(<tr key={d.id} className={`table-row-hover ${!d.inCurrentForecast?'opacity-50':''} ${i%2===0?'bg-white':'bg-gray-50/50'}`}>
          ...
          <td className="px-3 py-2.5 text-center">{pct(d.chancePrimary)}</td><td className="px-3 py-2.5 text-right">{fmt(calcWeighted30(d))}</td>
          <td className="px-3 py-2.5 text-center"><Badge text={`${d.monthsOpen??'?'}mo`} color={d.monthsOpen>6?'red':d.monthsOpen>3?'gold':'green'}/></td>
          <td className="px-3 py-2.5 text-center">{d.inCurrentForecast?<Badge text="Active" color="green"/>:<Badge text="Stale" color="gray"/>}</td>
        </tr>))}</tbody></table></TableWrapper>
      )}

      <SectionHeader title={`Open Partnerships (${pOpen.length})`} />
      {pOpen.length===0?<EmptyState message="No open partnership deals." icon="📋" />:(
        <TableWrapper><table className="w-full text-sm"><thead><tr className="bg-gray-100">
          <th className="px-3 py-2.5 text-left font-semibold">Deal</th><th className="px-3 py-2.5 text-right font-semibold">Total</th><th className="px-3 py-2.5 text-center font-semibold">Pipe</th>
          <th className="px-3 py-2.5 text-center font-semibold">Close%</th><th className="px-3 py-2.5 text-right font-semibold">30d$</th><th className="px-3 py-2.5 text-center font-semibold">Open</th><th className="px-3 py-2.5 text-center font-semibold">Status</th>
        </tr></thead><tbody>{pOpen.map((d,i)=>(<tr key={d.id} className={`table-row-hover ${!d.inCurrentForecast?'opacity-50':''} ${i%2===0?'bg-white':'bg-gray-50/50'}`}>
