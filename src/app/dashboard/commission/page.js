...
import { getRepCommissionForecast, REPS } from '@/data/utils';
import { StatCard, SectionHeader, fmt, pct, Badge, Select, FilterBar, TableWrapper } from '@/components/ui';
...
export default function Commission() {
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 mb-6 text-sm">Warm = 2.5% | Cold & Brave Digital = 3.5%. Top section shows forecast deals. Below, select additional deals you think you can close.</p>
      ...
      {forecastItems.length > 0 && (<>
        ...
        <TableWrapper><table className="w-full text-sm"><thead><tr className="bg-blue-50">
          <th className="px-3 py-2.5 text-left font-semibold">Deal</th><th className="px-3 py-2.5 text-left font-semibold">Type</th>
          <th className="px-3 py-2.5 text-center font-semibold">Pipeline</th><th className="px-3 py-2.5 text-right font-semibold">Value</th>
          <th className="px-3 py-2.5 text-center font-semibold">Rate</th><th className="px-3 py-2.5 text-center font-semibold">Close %</th>
          <th className="px-3 py-2.5 text-center font-semibold">Boosted %</th><th className="px-3 py-2.5 text-right font-semibold">Commission</th>
        </tr></thead><tbody>{forecastItems.sort((a,b)=>b.boostedCommission-a.boostedCommission).map((item,i)=>(
          <tr key={item.id} className={`table-row-hover ${i%2===0?'bg-white':'bg-gray-50/50'}`}>
            <td className="px-3 py-2.5 font-medium">{item.name}</td>
            <td className="px-3 py-2.5"><Badge text={item.type} color={item.type==='Assessment'?'blue':'gold'}/></td>
            <td className="px-3 py-2.5 text-center"><Badge text={item.pipeline} color={item.pipeline==='Warm'?'gold':'blue'}/></td>
            <td className="px-3 py-2.5 text-right">{fmt(item.value)}</td>
            <td className="px-3 py-2.5 text-center">{pct(item.rate)}</td>
            <td className="px-3 py-2.5 text-center">{pct(item.chancePrimary)}</td>
            <td className="px-3 py-2.5 text-center font-semibold">{pct(item.boostedPrimary)}</td>
            <td className="px-3 py-2.5 text-right font-semibold text-harbinger-700">{fmt(item.boostedCommission)}</td>
          </tr>))}</tbody></table></TableWrapper>
      </>)}

      <div className="bg-white rounded-xl shadow p-6 mb-6 mt-6 border border-gray-100">
      ...
      {nonForecastAssess.length > 0 && (<>
        ...
        <TableWrapper><table className="w-full text-sm"><thead><tr className="bg-gray-100">
          <th className="px-3 py-2.5 w-10">Close?</th><th className="px-3 py-2.5 text-left font-semibold">Deal</th>
          <th className="px-3 py-2.5 text-center font-semibold">Pipeline</th><th className="px-3 py-2.5 text-right font-semibold">Value</th>
          <th className="px-3 py-2.5 text-center font-semibold">Rate</th><th className="px-3 py-2.5 text-right font-semibold">If Closed</th>
        </tr></thead><tbody>{nonForecastAssess.sort((a,b)=>b.value-a.value).map((item,i)=>(
          <tr key={item.id} className={`${item.manualClose?'bg-green-50':i%2===0?'bg-white':'bg-gray-50/50'} cursor-pointer hover:bg-blue-50`} onClick={()=>toggle(item.id)}>
            <td className="px-3 py-2.5 text-center"><input type="checkbox" checked={manualCloses.has(item.id)} onChange={()=>toggle(item.id)} className="w-4 h-4 accent-harbinger-700 cursor-pointer" onClick={e=>e.stopPropagation()}/></td>
            <td className="px-3 py-2.5 font-medium">{item.name}</td>
            <td className="px-3 py-2.5 text-center"><Badge text={item.pipeline||'—'} color={item.pipeline==='Warm'?'gold':'blue'}/></td>
            <td className="px-3 py-2.5 text-right">{fmt(item.value)}</td>
            <td className="px-3 py-2.5 text-center">{pct(item.rate)}</td>
            <td className="px-3 py-2.5 text-right font-semibold text-green-700">{fmt(item.value*item.rate)}</td>
          </tr>))}</tbody></table></TableWrapper>
      </>)}
      ...
      {nonForecastPartner.length > 0 && (<>
        ...
        <TableWrapper><table className="w-full text-sm"><thead><tr className="bg-gray-100">
          <th className="px-3 py-2.5 w-10">Close?</th><th className="px-3 py-2.5 text-left font-semibold">Deal</th>
          <th className="px-3 py-2.5 text-center font-semibold">Pipeline</th><th className="px-3 py-2.5 text-right font-semibold">Total</th>
          <th className="px-3 py-2.5 text-right font-semibold">One-Time</th><th className="px-3 py-2.5 text-right font-semibold">Onboard</th>
          <th className="px-3 py-2.5 text-right font-semibold">Ongoing</th><th className="px-3 py-2.5 text-center font-semibold">Rate</th>
          <th className="px-3 py-2.5 text-right font-semibold">If Closed</th>
        </tr></thead><tbody>{nonForecastPartner.sort((a,b)=>b.value-a.value).map((item,i)=>(
          <tr key={item.id} className={`${item.manualClose?'bg-green-50':i%2===0?'bg-white':'bg-gray-50/50'} cursor-pointer hover:bg-blue-50`} onClick={()=>toggle(item.id)}>
            <td className="px-3 py-2.5 text-center"><input type="checkbox" checked={manualCloses.has(item.id)} onChange={()=>toggle(item.id)} className="w-4 h-4 accent-harbinger-700 cursor-pointer" onClick={e=>e.stopPropagation()}/></td>
            <td className="px-3 py-2.5 font-medium">{item.name}</td>
            <td className="px-3 py-2.5 text-center"><Badge text={item.pipeline||'—'} color={item.pipeline==='Warm'?'gold':item.pipeline==='Brave Digital'?'purple':'blue'}/></td>
            <td className="px-3 py-2.5 text-right font-semibold">{fmt(item.value)}</td>
            <td className="px-3 py-2.5 text-right text-gray-500">{fmt(item.oneTime)}</td>
            <td className="px-3 py-2.5 text-right text-gray-500">{fmt(item.onboarding)}</td>
            <td className="px-3 py-2.5 text-right text-gray-500">{fmt(item.ongoing)}</td>
            <td className="px-3 py-2.5 text-center">{pct(item.rate)}</td>
            <td className="px-3 py-2.5 text-right font-semibold text-green-700">{fmt(item.value*item.rate)}</td>
          </tr>))}</tbody></table></TableWrapper>
      </>)}
